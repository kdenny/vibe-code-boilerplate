...
- **Fail safe, not fast.** When a changed path under ``vibe/`` has no known
...
The mapping is convention-led: ``vibe/<pkg>/`` is covered by every
``tests/test_<pkg>_*.py`` plus ``tests/test_<pkg>.py``, and a top-level
``vibe/<name>.py`` is covered by ``tests/test_<name>.py``. The explicit
...
SOURCE_TEST_MAP: dict[str, tuple[str, ...]] = {
    ...
    "vibe/__main__.py": ("import_surface",),
    "vibe/cli/main.py": ("cli_main_pr", "cli_registry", "duplicate_pr_prevention"),
    "vibe/cli/registry.py": ("cli_registry",),
    "vibe/state.py": ("duplicate_pr_prevention",),
    ...
    "vibe/trackers/github_issues.py": ("trackers_github_issues",),
    "vibe/trackers/": (
        "trackers_linear",
        "trackers_shortcut",
...
# asserts each suite file exists and each participant lives under ``vibe/``.
...
def _tests_for_source(path: str, known_stems: set[str]) -> set[str] | None:
    ...
    (an unmapped ``vibe/`` change — fail safe).
    """
    ...
    if len(parts) == 1:
        # Top-level module: vibe/<name>.py -> tests/test_<name>.py
    ...
    # Package: vibe/<pkg>/... -> every test_<pkg>* suite.
    pkg = parts[0]
